...
def send_email(to,subject,template,**kwargs):
    msg =Message(app.config['FLASKY_MAIL_SUBJECT_PREFIX']+subject,
                sender=app.config['FLASKY_MAIL_SENDER'],recepients=[to])
    msg.body = render_template(template+'.txt',**kwargs)
    thr = Thread(target=send_async_emal,args=[app,msg])
    thr.start()
    mail.send(thr)
 
 ##I use dbForm template and a little bit changed function
 ##Just didn't want to mess dbs demo
